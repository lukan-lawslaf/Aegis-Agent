...
export default class Page {
  ...
  private async _addAntiDetectionScripts(): Promise<void> {
    ...
    // Injected into the page via Page.addScriptToEvaluateOnNewDocument.
    // Each patch is guarded: an error on one site must never surface as an
    // extension error or break the page.
    await this._puppeteerPage.evaluateOnNewDocument(() => {
      try {
        Object.defineProperty(navigator, 'webdriver', { get: () => undefined });
      } catch {
        /* site locked the property */
      }
  ...
  async takeScreenshot(fullPage = false): Promise<string | null> {
    if (!this._puppeteerPage) {
      // Firefox and restricted Chromium pages cannot be attached through
      // Puppeteer's extension transport. Use the native browser capture API
      // as a cross-browser fallback for sanitized visual context.
      if (!fullPage && this._validWebPage) {
        try {
          const { image } = await captureSanitizedVisibleTab(this._tabId);
          return image;
        } catch (error) {
          logger.error('Native tab screenshot failed:', error);
        }
      }
      throw new Error('Puppeteer page is not connected');
    }

    let privacyOverlayCount = 0;
    try {
      ...
      privacyOverlayCount = await this._puppeteerPage.evaluate(() => {
        ...
        for (const element of Array.from(document.querySelectorAll<HTMLElement>(selector))) {
          ...
          overlay.dataset.sihRedaction = 'credential';
        ...
        for (const element of Array.from(document.querySelectorAll<HTMLElement>('body *'))) {
          ...
          if (rect.width <= 0 || rect.height <= 0) continue;
          const overlay = document.createElement('div');
          overlay.dataset.sihRedaction = 'pii';
          Object.assign(overlay.style, {
            position: 'fixed',
            left: `${rect.left}px`,
            top: `${rect.top}px`,
      ...
      // Remove privacy overlays and animation override after encoding.
      if (privacyOverlayCount > 0) {
        await this._puppeteerPage.evaluate(() => {
          document.querySelectorAll('[data-sih-redaction]').forEach(node => node.remove());
        });
      }
      ...
      if (privacyOverlayCount > 0) {
        try {
          ...
          // Best-effort cleanup; the original screenshot never leaves this method.
        }
      }
